...
place_list = [
    # beach
    '용머리 해안'
    # ,'함덕 서우봉 해변','협재 해수욕장','월정리해변','중문 색달 해변',
...
# 파일을 저장하고
f = ['./곽지해수욕장.csv', './광치기 해변.csv']
read_f = []

#여기서 다 읽어오기
for file in f:
    df = pd.read_csv(file)
    df = df.drop(df.columns[[0]], axis = 'columns') # index 열 삭제
    read_f.append(df)

# 병합
merge_data = pd.concat(read_f, axis = 0, ignore_index = True)
print(merge_data)
...
"""for place in place_list:
    ...
    month_info = pd.concat([area_name_df,location_df,rating_df,month_df], axis=1)
    # with open('the most visited month.csv','a') as fd:
    #     fd.write(month_info)
    month_info.to_csv('the most visited month.csv')

    # f = open('the most visited month.csv','a', newline='')
    # wr = csv.writer(f)
    # wr.writerow(month_info)
